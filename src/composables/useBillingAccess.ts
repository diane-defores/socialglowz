...
export function useBillingAccess() {
  ...
  async function refreshAccess() {
    ...
    errorKey.value = null;
    try {
      access.value = await getConvexClient().query(api.billing.getProductAccess, {});
    } catch (error) {
  ...
  async function redeemCode(rawCode: string) {
    ...
    try {
      const result = await getConvexClient().mutation(api.billing.redeemCode, {
        code,
      });
  ...
  watch([isConvexConfigured, isAuthenticated, isAuthLoading], () => {
    void refreshAccess();
  }, { immediate: true });
