...
security_impact: yes
docs_impact: yes
user_story: "En tant qu'utilisatrice SocialGlowz qui achète via AppSumo, je veux saisir mon code dans l'app et voir immédiatement que mon accès fondateur est actif."
linked_systems:
...
Expose the processor-agnostic billing access layer inside SocialGlowz settings so AppSumo/manual customers can redeem a code without needing Lemon Squeezy, Polar, Stripe, Paddle, or another checkout provider.

## Scope

- Add a settings panel that reads `billing.getProductAccess`.
- Let authenticated users redeem AppSumo/manual codes through `billing.redeemCode`.
- Show clear states for inactive/free access, active founder access, loading, success, and recoverable errors.
...
- Authenticated users can enter a valid code from settings and receive active `founder_ltd` access.
...
| 2026-05-29 22:36:44 UTC | sf-ship | GPT-5 Codex | Prepared bounded ship for SocialGlowz redemption UI with explicit staging and targeted checks. | shipped | `Add AppSumo import runbook` |
