...
- Accès produit : `convex/billing.ts` est la couche agnostique processeur. L'app lit `billing.getProductAccess` via `src/composables/useBillingAccess.ts` et ne dépend pas directement d'AppSumo, Lemon Squeezy, Polar, Stripe, Paddle ou de la table legacy `subscriptions`. Les codes Lifetime Deal, early-bird, partenaires ou manuels sont importés via `billing.adminUpsertRedemptionCode` avec `SOCIALGLOWZ_BILLING_ADMIN_SECRET`, puis activés côté utilisateur avec `billing.redeemCode` depuis `BillingAccessPanel.vue` dans les réglages desktop/mobile. L'UI ne met pas en avant AppSumo afin de préserver le canal direct; les sources provider restent internes pour l'audit. Les codes restent en mémoire UI uniquement et les erreurs backend sont ramenées à des clés i18n sûres.
...
- `convex/billing.ts` : source de vérité d'accès produit, redemption Lifetime Deal/early-bird/manual et ledger billing agnostique.
